...
sc = SparkContext('spark://master:7077', 'spark_project')
# sc = SparkContext('local', 'spark_project')
sc.setLogLevel('WARN')
spark = SparkSession.builder.getOrCreate()

schemaString = "budget,genres,homepage,id,keywords,original_language,original_title,overview,popularity," \
...
moviesMapRdd = sc.textFile('hdfs://172.23.27.60:9000/user/hadoop/input/tmdb_5000_movies.csv').map(
    lambda line: Row(*next(csv.reader([line]))))
# moviesRdd = sc.textFile('input/tmdb_5000_movies.csv').map(lambda line: Row(*next(csv.reader([line]))))
# moviesRdd.take(10)
mdf = spark.createDataFrame(moviesMapRdd, schema)


def countByJson(field):
    flatMapRdd = mdf.select(field).filter(mdf[field] != '') \
        .rdd.flatMap(lambda g: [(v, 1) for v in map(lambda x: x['name'], json.loads(g[field]))])
    print(flatMapRdd.getNumPartitions())
    return flatMapRdd.repartition(1).reduceByKey(lambda x, y: x + y)


# 体裁统计
def countByGenres():
    genresRDD = countByJson("genres")
    res = genresRDD.collect()
...
def countByBudget(order='count', ascending=False):
    return mdf.filter(mdf["budget"] != 0).groupBy("budget").count().orderBy(order, ascending=ascending).toJSON().map(
        lambda j: json.loads(j)).take(10)
...
def distrbutionOfRuntime(order='count', ascending=False):
    return mdf.filter(mdf["runtime"] != 0).groupBy("runtime").count().orderBy(order, ascending=ascending).filter(
        'count>=100').toJSON().map(
...
def budgetVote():
    return mdf.select("title", "budget", "vote_average").filter(mdf["budget"] != 0).filter(
...
# 电影数量和评价的关系
def moviesVote():
    return mdf.filter(mdf["production_companies"] != '').filter(mdf["vote_count"] > 100).rdd.flatMap(
        lambda g: [(v, [float(g['vote_average']), 1]) for v in
                   map(lambda x: x['name'], json.loads(g["production_companies"]))]).repartition(1).reduceByKey(
        lambda x, y: [x[0] + y[0], x[1] + y[1]]).map(lambda v: [v[0], v[1][0] / v[1][1], v[1][1]]).collect()
